  ...
  return (
    <div>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'rgba(0,0,0,0.5)' }}>Manage all platform users</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
        <input
          ...
          style={{
            ...
            color: '#1A1A2E', fontSize: '0.9rem', outline: 'none', minWidth: '200px',
          }}
        />
        {(['all', 'student', 'teacher', 'admin'] as const).map(f => (
          <button
            ...
            style={{
              ...
              background: filter === f ? 'rgba(65, 105, 225, 0.2)' : 'rgba(0,0,0,0.03)',
              color: filter === f ? '#4169E1' : 'rgba(0,0,0,0.5)',
      ...
      {loading ? (
        ...
        <div style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.06)', borderRadius: '12px', overflow: 'hidden' }}>
          {filteredUsers.length === 0 ? (
            <div style={{ padding: '40px', textAlign: 'center', color: 'rgba(0,0,0,0.4)' }}>No users found</div>
          ) : (
            filteredUsers.map(user => (
              <div key={user.id} style={{ padding: '16px', borderBottom: '1px solid rgba(0,0,0,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.95rem', color: '#1A1A2E', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {user.full_name}
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'rgba(0,0,0,0.4)' }}>{user.email}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <select
                    ...
                    style={{
                      ...
                      background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)',
                      color: '#1A1A2E', fontSize: '0.8rem', cursor: 'pointer',
                    }}
                  >
